...
def unf_FVF_VB_m3m3_above(bob, cofb_1MPa, pb_MPa, p_MPa):
    ...
    ! Actually, this correlation is belonged ro Vasquez & Beggs (1980). In some sources is noted that this is Standing correlation.
...
def unf_density_oil_McCain(p_MPa,pb_MPa,co_1MPa, Rs_m3m3, gamma_gas, t_K, gamma_oil,gamma_gasSP = 0):
    """  
...
1995 correlation.
    ...
    t_F = 1.8*(t_K - 273.15) + 32
    if p_MPa < pb_MPa: 
        dro_p = ( 0.167 + 16.181 * 10**(-0.0425*ro_po))*(p_psia/1000) - 0.01 * (0.299+263*10**(-0.0603*ro_po))*(p_psia/1000)**2
        ro_bs = ro_po + dro_p
        dro_t = (0.00302 +1.505*ro_bs**(-0.951))*(t_F-60)**0.938 - (0.0216 - 0.0233 * 10**(-0.0161*ro_bs))*(t_F-60)**0.475
        ro_oR = ro_bs - dro_t
    else:
        pb_psia = p_MPa * 145.03773800721814
        dro_p = ( 0.167 + 16.181 * 10**(-0.0425*ro_po))(pb_psia/1000) - 0.01 * (0.299+263*10**(-0.0603*ro_po))*(pb_psia/1000)**2
        ro_bs = ro_po + dro_p
        dro_t = (0.00302 +1.505*ro_bs**(-0.951))*(t_F-60)**0.938 - (0.0216 - 0.0233 * 10**(-0.0161*ro_bs))*(t_F-60)**0.475
        ro_oRb = ro_bs - dro_t
...
def unf_undersaturatedoilviscosity_VB_cP(p_MPa,pb_MPa,bubblepointviscosity_cP):
...
def unf_undersaturatedoilviscosity_Petrovsky_cP(p_MPa,pb_MPa,bubblepointviscosity_cP):
    # viscosity correlation for pressure above bubble point
    # ref 1 Viscosity Correlations for Gulf of Mexico Crude Oils
    A = -1.0146 + 1.3322*np.log(bubblepointviscosity_cP) - 0.4876*np.log(bubblepointviscosity_cP)**2 - 1.15036*np.log(bubblepointviscosity_cP)**3
    p_psia = p_MPa * 145.03773800721814
    pb_psia = pb_MPa * 145.03773800721814
    viscosity_cP = bubblepointviscosity_cP + 1.3449 * 10**(-3)* (p_psia - pb_psia)*10**A
    return viscosity_cP

def unf_oil_viscosity_Beggs_VB_cP(deadoilviscosity_cP,Rs_m3m3,p_MPa,pb_MPa):
    ...
    if p_MPa<=pb_MPa:
        ...
        viscosity_cP = unf_undersaturatedoilviscosity_VB_cP(p_MPa, pb_MPa,saturatedviscosity_cP)
    return viscosity_cP
...
def unf_pseudocritical_properties_MPa_K(gamma_gas,Tc_H2S_K = 0,Tc_CO2_K = 0,Tc_N2_K = 0,pc_H2S_MPa = 1,pc_CO2_MPa = 1,pc_N2_MPa = 1,y_H2S = 0,y_CO2 = 0,y_N2 = 0):
    """
    correlation for pseudocritical properties
    ref 1 Piper, L.D., McCain, W.D., Jr., and Corredor, J.H. “Compressibility Factors for 
    Naturally Occurring Petroleum Gases.” Gas Reservoir Engineering. Reprint Series. Richardson, TX: SPE. Vol. 52 (1999) 186–200
    """
    if pc_H2S_MPa == 0 or pc_CO2_MPa == 0 or pc_N2_MPa == 0:
        print('Please,enter the correct pressure values')
        Tpc_K = 0
        ppc_MPa = 0
    else:
