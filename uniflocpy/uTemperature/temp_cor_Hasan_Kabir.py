"""
Температурная корреляция в проинтегрированном виде
TODO требуется создание этой же корреляции в форме градиента
"""
...
# TODO добавить учет угла наклона скважины
...
def __temp_fluid_c__(rhon, vm, at, cpm, u, rto_m, fc, tei_c, gg_cm, distance_m, tfi_c):
    ...
    return ((tei_c-gg_cm*distance_m) + (tfi_c - tei_c) ** np.exp(-distance_m/a) +
            a*(1-np.exp(-distance_m/a))*(gg_cm - const_g_m2sec / cpm + fc))
...
class Hasan_Kabir_cor2():
    ...
    def __temp_diff_an_iter__(self, delta):
        ...
        self.liquid_content = self.vsl_msec / self.vm_msec  # TODO это точно содержание жидкости, и можно его так определять
        self.rhon_kgm3 = self.rhol_kgm3 * self.liquid_content + self.rhog_kgm3 * (1 - self.liquid_content)
        self.mun_pas = self.mul_pas * self.liquid_content + self.mug_pas * (1 - self.liquid_content)
        self.kn = self.kl_wmc * self.liquid_content + self.kg_wmc * (1 - self.liquid_content)
        self.cpn = ((self.cpl_jkgc * self.rhol_kgm3 * self.liquid_content + self.cpg_jkgc *
                     self.rhog_kgm3 * (1 - self.liquid_content)) / self.rhon_kgm3)
